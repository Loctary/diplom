...
const Table1 = (props) => {
  const rows = props.names.map((name, idx) => ({ name: regionsEngUrk[name], value: props.values[idx] }));
  ...
  const headCells = [
    {
      id: 'name', numeric: false, disablePadding: true, label: 'Регіон',
    },
    {
      id: 'value', numeric: true, disablePadding: false, label: 'Індекс інвестиційної привабливості',
    },
  ];
      ...
      <div className={classes.root}>
        <Paper className={classes.paper}>
          ...
          <TablePagination
            ...
            labelDisplayedRows={({ from, to, count }) => `${from}-${to === -1 ? count : to} із ${count}`}
          />
        </Paper>
        <FormControlLabel
          control={<Switch checked={dense} onChange={handleChangeDense} />}
          label="Убрати відступи"
        />
      </div>
    );
  }
